...
void InsereInicio(TLista& Lista, TInfo Item){
    ...
    aux->Proximo = Lista.Primeiro->Proximo; // Serve para ficar com o mesmo endereço do primeiro,
                                            // porém é para ficar um nó a frente do primeiro.
    Lista.Primeiro->Proximo = p;
    
    if(p->Proximo == NULL)
        Lista.Ultimo = p;
    else // Só é para entrar nesse else se não for o primeiro item da lista a ser inserido
        //Lista.Primeiro->Proximo->Anterior = p; // Ao fazer isso vai pro nó cabeça.
        aux->Proximo->Anterior = p; // Se já houver itens na lista ele vai para o segundo que esse
                                    // recebe o endereço do primeiro.
}
